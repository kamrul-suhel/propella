          ...
          if(group.status < 1) {
            return
                ...
                key={group.id}
  			        axis="both"
  			        handle=".handle"
  			        defaultPosition={{x: containerWidth/100*group.positionX, y: containerHeight/100*group.positionY}}
  			        grid={[10, 10]}
  			        scale={1}
              ...
              >
  						<div handleid={group.id} className={`size-${group.icon_size}`}>
                {selectedDraggable === group.id &&
                  <div className="react-draggable-actions">
                    <Link className="button-round first" to={`/${url.projects}/${params.id}/groups/${group.id}/edit`}>
                    ...
                    <Link className="button-round fourth" to={`/${url.projects}/${params.id}/groups/${group.id}/competitors`}>
