...
export default class Edit extends React.PureComponent {
  ...
  triggerSubmit = () => this.formRef.submit()
	...
	handleSubmit = async () => {
    const { popup, params, person } = this.props
  ...
  popupActions = () => {
    const { step } = this.state
    const { person, params } = this.props

    switch (step) {
      case 1:
        return (
          [
            <Link to={`/${url.projects}/${params.id}/${url.groups}/${params.groupId}/${url.people}`} className="button">Cancel</Link>,
