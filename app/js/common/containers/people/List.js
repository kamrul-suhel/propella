  ...
  renderItem = (person) => {
    ...
    return (
      <li key={person.id}>
        ...
        <Link to={`/${url.projects}/${this.props.params.id}/${url.groups}/${this.props.params.groupId}/${url.people}/${person.id}`}>Edit</Link>
	...
	render() {
		...
		return (
			<Popup
        beforeTitle={<Link to={`/${url.projects}/${params.id}/${url.groups}/${params.groupId}/${url.organisations}`}>Organisations</Link>}
				title="People"
				closePath={`/${url.projects}/${params.id}/${url.groups}/${params.groupId}`}
        buttons={
          <React.Fragment>
            {!_.isEmpty(group.people) &&
              <Link className="button" to={`/${url.projects}/${params.id}/${url.groups}/${params.groupId}/${url.people}/add`}>Add person</Link>
            }
          </React.Fragment>
        }
			>
      ...
      >
				{_.isEmpty(group.people) ? (
					<Link to={`/${url.projects}/${params.id}/${url.groups}/${params.groupId}/${url.people}/add`}>Add your first person <span dangerouslySetInnerHTML={{__html: `&plus;`}} /></Link>
        ) : (
          <React.Fragment>
						<ul>
